...
if (url.indexOf('/chat/session/protect/status/get') >= 0 && obj.data) {
  obj.data.remainMsgCount = 99999;
}

if (url.indexOf('/chat/limit/friendly/check') >= 0 && obj.data) {
  obj.data.limit = false;
}

if (url.indexOf('/chat/limit/socialGraceScore') >= 0 && obj.data) {
  obj.data.graceScore = 999;
  obj.data.limit = false;
}

if (url.indexOf('/chat/limit/gift/give') >= 0 && obj.data) {
  obj.data.limit = false;
}

if (url.indexOf('/chat/limit/gift/info') >= 0 && obj.data) {
  obj.data.giftCount = 999;
  obj.data.limit = false;
}

if (url.indexOf('/chat/session/quickButtonCheck') >= 0 && obj.data) {
  obj.data.blocked = false;
}

if (url.indexOf('/chat/config/rounds') >= 0 && obj.data) {
  for (var key in obj.data) {
    if (typeof obj.data[key] === 'number') {
      obj.data[key] = 99999;
    }
  }
}

if (url.indexOf('/chat/check/privilegeExpireRemind') >= 0 && obj.data) {
  obj.data.needRemind = false;
}

if (url.indexOf('/robot/call/remainTimesAndSpeedCards') >= 0 && obj.data) {
  obj.data.remainTimes = 99999;
}
...
if (url.indexOf('/user/soulmate/status') >= 0 && obj.data) {
