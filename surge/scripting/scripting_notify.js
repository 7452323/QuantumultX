...
[Script]
scripting_push = type=event,event-name=notification,script-path=https://raw.githubusercontent.com/7452323/QuantumultX/main/surge/scripting/scripting_notify.js,argument="Scripting_key=xxx&group=Surge&automaticallyCopy=1&icon=url&sound=default&level=active&volume=5&ignoreRegex="

⚠️ 需先安装 Scripting App，工具 → Remote Push → 设置 → 获取 API Key
*/

const NAME = "Scripting通知";
let $ = new Env(NAME, { logLevel: 'info', log() {} });
...
function formatLog(obj, title) {
  const entries = Object.entries(obj).map(([key, value]) => {
    ...
    if (typeof value === 'object') value = formatObject(value);
    return `├ ${key}:${value}`;
  }).filter(Boolean);
...
function formatObject(obj) {
  const entries = Object.entries(obj).map(([key, value]) => {
    if (typeof value === 'object' && value !== null) value = formatObject(value);
