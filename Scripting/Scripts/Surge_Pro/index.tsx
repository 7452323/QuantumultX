...
function Dashboard({ config, onOpenSettings, onOpenWidgetSettings, onChangeRefreshSeconds }: { config: Config; onOpenSettings: () => void; onOpenWidgetSettings: () => void; onChangeRefreshSeconds: (seconds: number) => void }) {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      let nextModel: PanelModel | null = null
      setModel(currentModel => {
        const hasSnapshot = currentModel.connected || currentModel.updatedAt > 0
        nextModel = hasSnapshot
          ? { ...currentModel, connected: false, error: `${message} · 显示上次数据` }
          : offlineModel(config, error, loadHistory())
        return nextModel
      })
      if (nextModel) {
        saveWidgetCache(nextModel)
        if (webLoaded.current) await webView.current.evaluateJavaScript(`window.applyNativeUpdate(${webJSON(nextModel)})`)
      }
