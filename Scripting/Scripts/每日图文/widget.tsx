...
function TextOnlyWidget(props: { text: string; source: string }) {
  ...
  const md = f === "systemMedium"
  return (
    <VStack padding={{ horizontal: sm ? 10 : 14, vertical: sm ? 8 : 10 }} background="#1a1a2e">
      <Text font={sm ? 8 : md ? 9 : 11} foregroundStyle="rgba(255,255,255,0.55)" padding={{ horizontal: sm ? 8 : 12, vertical: sm ? 4 : 6 }}>每日一言一图</Text>
      <Spacer />
      <Text font={sm ? 10 : md ? 12 : 15} foregroundStyle="rgba(255,255,255,0.92)" fontWeight="medium" lineLimit={sm ? 5 : md ? 4 : 8}>{props.text}</Text>
      {props.source ? <Text font={sm ? 9 : md ? 10 : 12} foregroundStyle="rgba(255,255,255,0.55)">—— {props.source}</Text> : null}
    </VStack>
  )
}
