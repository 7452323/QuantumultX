...
async function fetchHTML(url: string): Promise<string> {
  ...
  try {
    await webView.loadURL(cleanUrl)
    // Cloudflare JS challenge may trigger a redirect — wait for final page
    await delay(2000)
    await webView.waitForLoad()
    const html = await webView.getHTML()
    if (!html || html.length < 200) throw new Error('页面加载失败')
    // Check if still trapped on Cloudflare challenge page
    if (html.includes('cf-browser-verification') || html.includes('_cf_chl_opt') || html.includes('cf_challenge')) {
      // Try waiting longer for the challenge to resolve
      await delay(3000)
      await webView.waitForLoad()
      const retryHtml = await webView.getHTML()
      ...
      if (retryHtml.includes('cf-browser-verification') || retryHtml.includes('_cf_chl_opt')) {
        throw new Error('被 Cloudflare 拦截，请稍后再试')
      }
      return retryHtml
    }
    return html
...
function LongNovelListView() {
  ...
  async function loadBooks() {
    try {
      let html = await fetchHTML(`${LONG_NOVEL_BASE}/p/all.html`)
      let items = extractBookList(html)
      if (!items.length) {
        html = await fetchHTML(LONG_NOVEL_BASE)
